...
function ibksim(;it::Bool=false)
	...
	if it
		mp = @manipulate for dosim = Dict("sim" => true, "sol" => false),
			id = Dict("id=$id" => id for id in 1:2),
			nsims = spinbox(label="nsims"; value=20) |> onchange,
			ti in slider(times, label = "period", value = 1) |> onchange,
			γ in slider(gammas, label = "γ", value =p.gamma ) |> onchange,
			β in slider(betas, label = "β", value =p.beta)  |> onchange,
			a in slider(alphas, label = "α", value = p.alpha)  |> onchange,
			aT in slider(alphaT, label = "αT", value = p.alphaT)  |> onchange,
			alowT in slider(-5:0.5:0.0, label = "alowT", value = p.a_lowT)  |> onchange,
			λ in slider(lambdas, label = "λ", value =p.lambda)  |> onchange
			# ρ in slider(rhos, label = "ρ", value =p.ρ)
			pp = Dict(:nsims => nsims,:nT => 70, :retage => 55, :pension => 0.5,:a_low => -5.0,:a_lowT => alowT:beta => β, :alphaT => aT, :alpha => a, :gamma => γ , :lambda => λ)
			m,p = runbk(par = pp)
			if dosim
				s = sim(m,p)
				plot_s(s)
