...
mutable struct GModel <: Model
	...
	function GModel(p::Param)
		...
		this.m1 = Dict(it => Dict(id => Float64[(this.avec[ia] * p.R)  .+ (income(it,p,this.yvec[iy]) * (id==1))  .+ (p.pension * (id==2)) for iy in 1:p.ny , ia in 1:p.na ] for id=1:p.nD) for it=2:p.nT)
		this.c1 = zeros(p.na,p.ny)
		this.ev = zeros(p.na,p.ny)
